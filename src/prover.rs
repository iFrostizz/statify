use crate::{
    bytecode::Mnemonics,
    data::{to_word, EVMStack, Memory, RevertReason, Stack},
    opcodes::OpCodes::*,
    z3::word_to_bv,
};
use z3::{
...
impl<'a> Prover<'a> {
    ...
    pub fn run(&mut self) -> Result<Optimize, RevertReason> {
        ...
        for instruction in self.code {
            ...
            match opcode {
                ...
                Calldataload => {
                    let off = stack.pop_bv(&self.ctx, "off")?;
                }
                op => todo!("{:?}", op),
            }
        }
